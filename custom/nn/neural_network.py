...
def cat_cross_entropy(pred: cp.ndarray, target: cp.ndarray) -> Tuple[cp.ndarray, cp.ndarray]:
    error = -cp.mean(cp.sum(target * cp.log(pred), axis=1))
    gradient = (pred - target) / len(pred)
    return error, gradient


def mse(pred: cp.ndarray, target: cp.ndarray) -> Tuple[cp.ndarray, cp.ndarray]:
    error = cp.mean((pred - target) ** 2, axis=0)
    gradient = (2 * (pred - target)) / len(pred)
    return error, gradient


def normalize(data: cp.ndarray) -> cp.ndarray:
...
class Layer:
    ...
    @abstractmethod
    def backward(self, output_gradient: cp.ndarray, learn_rate: float) -> cp.ndarray:
        pass
...
class Dense(Layer):
    ...
    def backward(self, output_gradient: cp.ndarray, learn_rate: float) -> cp.ndarray:
        ...
        self.__bias -= learn_rate * cp.sum(output_gradient, axis=0, keepdims=True)
        return ret
...
class Convolution(Layer):
    ...
    def correlate(cls, a: cp.ndarray, b: cp.ndarray):
        ...
        return cp.tensordot(temp, b, axes=3)

    @classmethod
    def windows(cls, a: cp.ndarray, window_shape) -> cp.ndarray:
    ...
    def backward(self, output_gradient: cp.ndarray, learn_rate: float) -> cp.ndarray:
        bias_grads = cp.sum(output_gradient, axis=(0, 1, 2))

        pad_amt = self.__kernel_size - 1
        padded_grads = cp.pad(output_gradient, ((0, 0), (pad_amt, pad_amt), (pad_amt, pad_amt), (0, 0)),
        ...
        kernel_grads = cp.tensordot(input_windows, output_gradient, axes=((0, 3, 4), (0, 1, 2)))
...
class MaxPool(Layer):
    ...
    @classmethod
    def windows(cls, inp: cp.ndarray, window_shape) -> cp.ndarray:
    ...
    def forward(self, inp: cp.ndarray) -> cp.ndarray:
        ...
        if self.__inp_shape[1] % 2 == 1 and self.__inp_shape[2] % 2 == 1:
            inp = cp.pad(inp, ((0, 0), (0, 1), (0, 1), (0, 0)), mode="constant", constant_values=0)
        elif self.__inp_shape[1] % 2 == 1:
            inp = cp.pad(inp, ((0, 0), (0, 1), (0, 0), (0, 0)), mode="constant", constant_values=0)
        elif self.__inp_shape[2] % 2 == 1:
            inp = cp.pad(inp, ((0, 0), (0, 0), (0, 1), (0, 0)), mode="constant", constant_values=0)
        ...
        self.__mask = cp.isclose(windows, mx)
        return mx.squeeze()
...
class LeakyRelu(Layer):
    ...
    def forward(self, inp: cp.ndarray) -> cp.ndarray:
        self.__input = inp
        return cp.maximum(inp, inp * self.__alpha)

    def backward(self, output_gradient: cp.ndarray, learn_rate: float) -> cp.ndarray:
        temp = cp.ones(self.__input.shape)
...
class Tanh(Layer):
    ...
    def backward(self, output_gradient: cp.ndarray, learn_rate: float) -> cp.ndarray:
        return (1 - cp.tanh(self.__input) ** 2) * output_gradient
...
class SoftMax(Layer):
    ...
    def forward(self, inp: cp.ndarray) -> cp.ndarray:
        prep = inp - cp.max(inp, axis=1, keepdims=True)
        self.__output = cp.exp(prep) / cp.sum(cp.exp(prep), axis=1, keepdims=True)
        return self.__output

    def backward(self, output_gradient: cp.ndarray, learn_rate: float) -> cp.ndarray:
        # batch_size = output_gradient.shape[0]
        # classes = output_gradient.shape[1]
        # eyes = cp.tile(cp.eye(classes), (batch_size, 1, 1))
        # jacob_matrix = cp.einsum("ij,ijk->ijk", self.__output, eyes)\
        #     - cp.einsum("ij,ik->ijk", self.__output, self.__output)
        # return cp.einsum("ij,ijk->ik", output_gradient, jacob_matrix)
...
class Module:
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        return self._forward_raw_data(cp.asarray(self.normalize(x))).get()
    ...
    def fit(self, x: np.ndarray, y: np.ndarray, error_fn, lr, batch_size: int = 1, max_epochs: int = 10000):
        ...
        x = self.normalize(x)
        # indexes = cp.arange(0, x.shape[0])
        for epoch in range(max_epochs):
            # cp.random.shuffle(indexes)
            ...
            for idx in range(0, len(x), batch_size):
                x_batch = cp.asarray(x[idx:idx + batch_size])
                y_batch = cp.asarray(y[idx:idx + batch_size])
            ...
            print(f"\rEpoch {epoch}, Error: {total_error / batches}", end="")
        print()
